...
fetch("https://api.open-meteo.com/v1/forecast?latitude=55.7&longitude=11.66&daily=weather_code&current=apparent_temperature&timezone=Europe%2FBerlin&past_days=1&forecast_days=16")
    ...
    if (todayIndex !== -1) {
        ...
        const icon = getWeatherIcon(codes[todayIndex]);
        const tempIcon = getTempIcon(temp);
        const tempDisplay = `${tempIcon} ${Math.round(temp)}°C`;
        todayBox.innerHTML = `<div class="tile today">${icon}<br>I dag<br>${capitalisedDay}<br>${tempDisplay}</div>`;
